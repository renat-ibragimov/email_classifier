...
class FakeMessage:
    ...
    def __init__(self, text=None, document=None, user_id=USER_ID):
        self.text = text
        self.caption = None
...
class TestCommands:
    async def test_help_defaults_to_ukrainian(self):
        message = FakeMessage()
    ...
    async def test_lang_toggles_to_english(self):
        message = FakeMessage()
    ...
    async def test_lang_toggles_back(self):
        ...
        assert language_store.get_language(USER_ID) is LanguageEnum.UK

    async def test_help_follows_the_chosen_language(self):
        message = FakeMessage()
...
class TestText:
    async def test_classifies_and_answers_with_a_card(self):
        ...
        assert language is LanguageEnum.UK
    ...
    async def test_duplicate_adds_the_stored_result_line(self):
        ...
        assert "уже перевіряли" in message.replies[0]
    ...
    async def test_blank_text_is_rejected_before_the_api(self):
        ...
        assert message.replies == [t(LanguageEnum.UK, "empty_text")]
...
class TestErrorMapping:
    ...
    async def test_each_error_gets_its_own_line(self, error, key):
        ...
        assert message.replies == [t(LanguageEnum.UK, key)]
...
class TestDocument:
    ...
    async def test_non_eml_gets_the_hint(self):
        ...
        client.classify.assert_not_awaited()
        assert message.replies == [t(LanguageEnum.UK, "hint")]
    ...
    async def test_oversized_file_is_refused(self):
        ...
        client.classify.assert_not_awaited()
        assert message.replies == [t(LanguageEnum.UK, "file_too_large")]
    ...
    async def test_failed_download_is_reported_as_unavailable(self):
        ...
        client.classify.assert_not_awaited()
        assert message.replies == [t(LanguageEnum.UK, "error_unavailable")]
